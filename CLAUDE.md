...
| **Per-copy duplicator `shapeId` (UNSOLVED region swap)** | A duplicator with multiple shapes **cycles them by copy index** (checkerboard). A true per-copy *region* swap (BLOXEL: `Mask → isWithin(invert) → numberRange → shapeId`, one duplicator) could **not** be reproduced from scratch: a duplicator evaluates `isWithin` at its **own pivot** (one position → uniform), confirmed by the [docs](https://cavalry.studio/docs/nodes/behaviours/is-within/) ("evaluates based on the shape's pivot position") + ~10 render probes. Ruled out as the per-copy enabler: `shapePosition` drive, `sortDistribution` mode 2, springs, `useIndex`, `indexContext` (default already 0), and a JS expression (`javaScriptUtility` isn't a creatable type here). The BLOXEL swap is likely **inseparable from its animated push rig** (oscillator displacing copies so their pivots differ). Static region swap = open problem; revisit with the live file. |
| **~~Region shape-swap (two-grid)~~ — REJECTED** | Two grids (base everywhere + region clipped to mask, drawn on top) *looks* close but is a **masking trick, not the effect**: it leaves clipped half-cells at the boundary instead of full per-cell shapes. User rejected it. Do **not** ship this as "Shape Swap". |
...
| **`isWithin` evaluates at the duplicator pivot, not per-copy** | Connecting `isWithin → numberRange → shapeId` gives a uniform value for ALL copies — even with `sortDistribution` + `useIndex:true`. Driving `shapePosition` from a `getVector`/`value2`/`round` chain did **not** flip it (the chain itself evaluates at the pivot → 0). See the "Per-copy duplicator `shapeId`" row — making this per-copy from scratch is an open problem. |
...
The **Grid FX** section + registry (`typemap.gridPresets`, `Engine.buildGridPreset`, `panel._buildReqSection`/`_presetSpec`) is generalised, shared machinery that mirrors Text Presets — but currently ships **no preset** (the registry is empty, so the panel skips the section). Shape Swap Grid was shelved (see backlog).

The first two presets are thin (discovery was the work; ~30-line builders) because the prerequisite/stub/checklist machinery is shared. Highlight Words shows the **parameterised** path: pre-build a fixed set of rows (Cavalry builds layout once — reveal hidden rows instead of adding them) and collect their values at build time.

- **Quick Actions** (contextual, top of the panel): the ~5 most relevant one-click actions for the **current selection**, C4D-quick-duplicate style. Declarative: `typemap.contextActions` = `{ byCategory[cat] -> [actionKey...], defs[key] -> {label,hint} }`. `Selection.classify(id)` / `classifySelection(ids)` map the selection to a category (`shape`/`text`/`cloner`/`effector`/`field`/`image`/`multiShapes`/`multi`/`other`) from concrete `getLayerType`/`isShape`, with `getSuperTypes` only as a coarse `Shape` fallback. `Engine.runContextAction(key, sel)` is the dispatcher: generic verbs wrap documented `api.*` (`duplicate`, `preCompose`, `centrePivot`, group via `create("group")`+`parent`, `deleteLayer`) and type-specific keys **route to existing Engine features** (`createCloner`/`applyPreset`/`addEffector`/`addField`/`buildTextPreset`/`highlightWords`/`buildRig`/`setEffectorMuted`), returning a normalised `{ ok, select?, message?, missing?, stubbed?, imageUsed? }`. The panel builds a **fixed pool of 6 buttons once** (`Panel._qaButtons`) and relabels/rewires/show-hides them per selection in `_refreshQuickActions` (same Cavalry-lays-out-once trick as Highlight rows; buttons support `setHidden` directly — see `_pathBtn`). Additive section — the static button walls are unchanged.
  - **Verify-live item:** the generic verbs + type routes reuse already-verified code, but the exact `getSuperTypes()` return strings are **assumed `["Shape"]`-ish** and not yet live-confirmed (nothing critical depends on them — concrete-type checks decide first). Run `node tools/verify_context.js` (Stallion open) to confirm `getSuperTypes`/`duplicate`/`preCompose`/`centrePivot` and the action wiring with zero warnings.

---

## Deferred / backlog

- **Shape Swap Grid — SHELVED (revisit with the live file).** The real BLOXEL effect is a true per-copy `shapeId` swap in ONE duplicator; reproducing it from scratch is unsolved (see the "Per-copy duplicator `shapeId`" facts row — `isWithin` evaluates at the duplicator pivot, and the swap looks tied to the animated push rig). The shipped two-grid mask version was a misleading trick and has been **removed** from the panel (registry `gridPresets` is empty; the Grid FX section is hidden when empty; engine `_buildShapeSwap`/`_gridOf` and grid stubs deleted; generic `buildGridPreset`/`_buildReqSection` machinery kept dormant). Tools for revisiting: `node tools/dump_scene_full.js` → `tools/scene_full.json` (the full 204-layer dump), and the `tools/probe_*.js` render-probes. **Open question to settle first: is the example's region swap static or a product of the push animation?** (User unsure.)
